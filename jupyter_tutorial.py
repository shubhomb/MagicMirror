...
CURR_POSE = input('What pose are we training? 0- Swipe right, 1- Swipe left, 2- ok, 3- up, 4- down, 5- crossed')
CURR_POSE = classes[int(CURR_POSE)]
DATA =  'gestures'

if not os.path.exists(DATA):
    print ('Making image save file %s'%DATA)
    os.mkdir(DATA)
else:
    print ('Image save directory already exists: %s'%DATA)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        cv2.putText(foreground_display, "FPS : " + str(int(fps)), positions['fps'], cv2.FONT_HERSHEY_SIMPLEX, 0.65,
                    (50, 170, 50), 2)
        cv2.putText(display, "FPS : " + str(int(fps)), positions['fps'], cv2.FONT_HERSHEY_SIMPLEX, 0.65, (50, 170, 50),
                    2)
        ...
        if k == 27:
            ...
            # s pressed
            img_count += 1
            if not os.path.exists(os.path.join(DATA,CURR_POSE)):
                os.mkdir((os.path.join(DATA,CURR_POSE)))
            fname = os.path.join(DATA, CURR_POSE, "{}_{}.png".format(CURR_POSE, img_count))
            imwrote = cv2.imwrite(fname, hand_crop)
            if imwrote:
                print ('Saved image %s'%fname)
            else:
                print ('Failed to save image %s' %fname)
            if img_count>IMGLIMIT:
                break
        elif k != 255:
            print(k)
